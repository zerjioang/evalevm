...
# Run perf stat and time on the command
# Redirect tool's stdout and stderr to the script's stdout/stderr directly
# Perf outputs to TMP_PERF, time outputs to TMP_OUT
/usr/bin/time -f 'max_ram_kb=%M\nuser_seconds=%U\nsys_seconds=%S\nreal_seconds=%e\nexit_code=%x' -o "$TMP_OUT" \
  perf stat -x, -o "$TMP_PERF" -- "$@"
